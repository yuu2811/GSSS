...
世界トップクラスの投資銀行・ヘッジファンドのスタイルで日本株を分析する12種類の分析エンジンを搭載したWebアプリです。銘柄コードを入力するだけで、プロフェッショナルレベルの分析レポートを即座に生成します。
...
| 11 | **Academic Paper 定量分析** | Fama-French、モメンタム、低ボラティリティ、QMJ等の学術論文ベースファクター分析 | 必要 |
| 12 | **チャートパターン分析** | ヘッドアンドショルダー、ダブルトップ、三角保ち合い等のパターン認識とトレンド分析 | 必要 |
...
# または pyproject.toml を使用（開発用ツール含む）
pip install -e ".[dev]"
```
...
├── requirements.txt                 # Python依存パッケージ（pip用）
├── .gitignore
│
├── analyzers/                       # 分析エンジン群
│   ├── __init__.py                  # 公開API（__all__）
│   ├── stock_data.py                # yfinance データ取得・テクニカル指標計算
│   ├── scoring.py                   # 共通スコアリングユーティリティ
...
│   ├── academic_quant.py            # 学術論文ベース定量分析
│   └── chart_pattern.py             # チャートパターン認識
│
├── tests/                           # テストスイート（pytest）
│   ├── conftest.py                  # 共通フィクスチャ・モックデータ
│   ├── test_scoring.py              # scoring.py 単体テスト
│   ├── test_stock_data.py           # StockDataFetcher ヘルパーテスト
│   ├── test_analyzers.py            # 各アナライザーのスモークテスト
│   └── test_app.py                  # Flask APIエンドポイントテスト
│
